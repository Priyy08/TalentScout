...
# --- Database Setup ---
def init_database():
    """Initialize SQLite database and create candidates and candidate_answers tables."""
    try:
        ...
        """)
        conn.commit()
        logger.info("Database initialized successfully with candidates and candidate_answers tables.")
...
def save_candidate_data(candidate_info=None, answers=None, candidate_id=None):
    """Save candidate information and/or their answers to SQLite database."""
    try:
        conn = sqlite3.connect("candidates.db")
        cursor = conn.cursor()
        
        # Verify candidates table exists, reinitialize if not
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='candidates'")
        if not cursor.fetchone():
            logger.warning("Candidates table not found. Reinitializing database.")
            init_database()
        
        # Insert candidate information if provided
        if candidate_info:
            cursor.execute("""
                INSERT INTO candidates (
                    full_name, email_hash, phone_hash, years_experience,
                    desired_position, current_location, tech_stack, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                candidate_info.get("full_name", ""),
                hash_sensitive_info(candidate_info.get("email", "")) if candidate_info.get("email") else "",
                hash_sensitive_info(candidate_info.get("phone_number", "")) if candidate_info.get("phone_number") else "",
                candidate_info.get("years_experience", ""),
                candidate_info.get("desired_position", ""),
                candidate_info.get("current_location", ""),
                candidate_info.get("tech_stack", ""),
                datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ))
            candidate_id = cursor.lastrowid
            logger.info(f"Saved candidate data for ID {candidate_id}.")
        
        # Insert answers if provided and candidate_id is available
        if answers and candidate_id:
            for question, answer in answers:
                cursor.execute("""
                    INSERT INTO candidate_answers (
                        candidate_id, question, answer, created_at
                    ) VALUES (?, ?, ?, ?)
                """, (
                    candidate_id,
                    question,
            ...
            logger.info(f"Saved {len(answers)} answers for candidate ID {candidate_id}.")
        
        conn.commit()
        return candidate_id
    except sqlite3.Error as e:
        logger.error(f"Error saving candidate data: {e}")
        st.error(f"Failed to save candidate data: {e}", icon="❌")
        return None
    finally:
        conn.close()
...
try:
    ...
    logger.info("VADER lexicon found locally.")
    _sentiment_analyzer = SentimentIntensityAnalyzer()
    logger.info("Sentiment Analyzer initialized.")
except LookupError:
    logger.info("VADER lexicon not found locally. Attempting download...")
    try:
        nltk.download('vader_lexicon')
        _sentiment_analyzer = SentimentIntensityAnalyzer()
        logger.info("VADER lexicon downloaded and Sentiment Analyzer initialized.")
    except Exception as download_e:
        logger.warning(f"Failed to download VADER lexicon: {download_e}")
        st.warning(f"Failed to download VADER lexicon: {download_e}. Sentiment analysis will be disabled.", icon="⚠️")
except Exception as e:
    logger.warning(f"An error occurred during NLTK setup: {e}")
    st.warning(f"An error occurred during NLTK setup: {e}. Sentiment analysis will be disabled.", icon="⚠️")
...
try:
    ...
    st.error(f"Error initializing Groq client: {e}. Please check your GROQ_API_KEY in the .env file.", icon="❌")
...
def get_llm_response(prompt, context_messages):
    ...
    if not llm_ready_flag: return "LLM client not initialized."
    messages = [{"role": "system", "content": ASSISTANT_PERSONA}] + context_messages + [{"role": "user", "content": prompt}]
    try:
        response = client.chat.completions.create(
            model=MODEL_NAME, messages=messages, temperature=0.5, max_tokens=250
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
        st.error(f"Error communicating with Groq API: {e}", icon="📡")
...
def generate_technical_questions(tech_stack, context_messages):
    ...
    try:
        response = client.chat.completions.create(
            model=MODEL_NAME, messages=messages, temperature=0.6, max_tokens=350
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
        st.error(f"Error generating technical questions via Groq: {e}", icon="❔")
...
st.caption("Enhanced with Sentiment Analysis, Database Storage & UI Improvements | Powered by Groq ⚡")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                if current_stage == "get_name":
                    ...
                    st.markdown(tech_stack_confirmation)
                    ...
                    # Move to end with concluding remarks
